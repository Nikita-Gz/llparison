...
def _load_raw__basic_math_data() -> Dict[str, str]:
  ...
  for left_value in sampled_range:
    for right_value in sampled_range:
      # uses a plus sign only when the number on the right is positive
      plus_sign = '+'
      if right_value < 0:
        plus_sign = ''
      
      input_id = f'{left_value}:{right_value}'
      equation = f'{left_value}{plus_sign}{right_value}='
      final_equations[input_id] = equation
...
def load_appropriate_dataset_for_task(task_type: TaskType) -> Any:
  ...
  appropriate_data_loader_for_task = {
    ...
    TaskType.BASIC_MATH: _load_raw__basic_math_data
  }.get(task_type, None)
